...
d3.csv("https://data1500.github.io/test-project/providenceTemp.csv", (data) => {
  // Add X axis
  let x = d3.scaleLinear().domain([1945, 2023]).range([0, width]);
  ...
  // Add Y axis
  let y = d3.scaleLinear().domain([54, 66]).range([height, 0]);

  svg.append("g").call(d3.axisLeft(y));

  let colors = d3.scaleLinear().domain([54, 66]).range(["#1f78b4", "#e31a1c"]);
  ...
  let dots = svg
    ...
    .on("mouseout", function () {
      d3.select(this).transition().duration(400).attr("r", 8);
